...
📐 **Design phase complete — implementation not yet started.** The documents in [`docs/`](./docs) are the full specification this project will be built against. `Requirements.md` and `SDD.md` are the **living** versions — kept in sync with reality as implementation proceeds (see `CLAUDE.md` for the update rule). The original `.docx` files are preserved as the frozen v1 baseline/historical record and are not edited after this point.

| Document | What it defines |
|---|---|
| [Requirements Specification (living)](./docs/Requirements.md) — [frozen v2.1 baseline](./docs/Scattered%20Oaks%20Farm%20Requirements%20v2.docx) | What the site must do — public site behavior, the Administrator CMS, security/password policy, the deployment approval gate, and the design-iteration workflow. Combines the original owner-authored requirements with everything defined in the approved Claude Design prototype, plus additional requirements identified during analysis (each tagged `[PDF]` / `[DESIGN]` / `[ADDED]` / `[AMENDED]` for provenance) — includes a change log at the bottom. |
| [Software Design Description (living)](./docs/SDD.md) — [frozen v1.0 baseline](./docs/Scattered%20Oaks%20Farm%20Software%20Design%20Description.docx) | How it will be built — concrete architecture, frontend/backend component design, the D1 data model, the full API contract, security design, the CI/CD pipeline, testing strategy, and a step-by-step manual configuration checklist for everything that has to be set up by hand in GitHub, Cloudflare, and Resend before the pipeline can run. Includes a change log at the bottom. |
...
| [Manual Setup Guide](./docs/Manual-Setup-Guide.md) | Exact click-paths and commands for every one-time GitHub/Cloudflare/Resend setup step in SDD §10, reordered by real dependency instead of by platform. |
